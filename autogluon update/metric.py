...
####added
    elif metric.name in [sensitivity]:
        return recall_score(metric_data[Y_TRUE], metric_data[Y_PRED])

    elif metric.name in [specificity]:
        cm = confusion_matrix(metric_data[Y_TRUE], metric_data[Y_PRED])
